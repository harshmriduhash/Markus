...
const generateProjectWithAI = async (
  ...
  if (
    ...
    !prompt.toLowerCase().includes("clean ui")
  ) {
  ...
  addMessage(
    `I'll help you build a web application based on your request: "${prompt}". Let me analyze what you need and create the appropriate files.`
  );
  ...
  try {
    ...
    if (previousFiles && previousFiles.length > 0) {
      const previousCode = previousFiles
    ...
    try {
      ...
      addMessage(
        "I've received a response from the AI model. Now I'll process the data and create your web application."
      );
      ...
      try {
        ...
        throw new Error(
          "Failed to parse the AI model response. Please try again."
        );
      }
      ...
      if (parsedData.explanation) {
        ...
        addMessage(
          "I've created the basic project structure. Now I'll add styling and functionality."
        );
      }
      ...
      if (cssFile) {
        ...
        updateStep(step3Id, {
          completed: true,
          message:
            "No separate CSS file found. Styles might be included in the HTML.",
        });
      }
      ...
      if (jsFile) {
        ...
        addMessage(
          "I've added JavaScript to make the application interactive."
        );
      } else {
        updateStep(step4Id, {
          completed: true,
          message:
            "No separate JavaScript file found. Scripts might be included in the HTML.",
        });
      }
      ...
      let errorMessage =
        "Failed to connect to the AI service. Please check your API key and try again.";
      ...
      if (axios.isAxiosError(axiosError)) {
        if (axiosError.response) {
          ...
          if (status === 401 || status === 403) {
            ...
            errorMessage =
              "The API endpoint could not be found. This model might not be available.";
          } else if (status === 429) {
          ...
          errorMessage =
            "No response received from the AI service. Please check your internet connection.";
        } else {
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  updateGenerationStep: (id, updates) =>
    set((state) => ({
